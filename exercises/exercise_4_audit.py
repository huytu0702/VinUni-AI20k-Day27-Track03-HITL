...
Completed implementation emits one AuditEntry per node for
...
async def node_route(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=decision,
        reason=state["analysis"].confidence_reasoning,
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=state.get("human_choice") or "escalate",
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason="; ".join(f"{question}: {answer}" for question, answer in answers.items()),
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
