    ...
    async with db_conn() as conn:
        async with conn.execute(
            ...
            SELECT thread_id,
                   ...
                   CASE MAX(CASE risk_level WHEN 'high' THEN 3 WHEN 'med' THEN 2 WHEN 'low' THEN 1 ELSE 0 END)
                       WHEN 3 THEN 'high'
                       WHEN 2 THEN 'med'
                       WHEN 1 THEN 'low'
                       ELSE 'unknown'
...
with st.sidebar:
    ...
    if recent_sessions:
        st.dataframe(pd.DataFrame(recent_sessions), hide_index=True, use_container_width=True)
    else:
...
def render_escalation_card(payload: dict) -> dict | None:
    ...
    with st.form("escalation"):
        ...
        for idx, question in enumerate(payload["questions"]):
            answers[question] = st.text_input(question, key=f"escalation_answer_{idx}").strip()
        if st.form_submit_button("Submit answers"):
            if all(answers.values()):
                return answers
            st.error("Please answer every escalation question before continuing.")
    return None
